...
def main():
    ...
    for i in range(1,10):
        row_data = []
        patern = path_to_dct(f"./images/C01_IT_new/uzo_cv01_IT_im0{i}.jpg")
        for j in range(1,10):
            to_compare = path_to_dct(f"./images/C01_IT_new/uzo_cv01_IT_im0{j}.jpg")
...
def path_to_dct(path:str):
    image = cv2.imread(path,cv2.COLOR_BGR2RGB)
    image = np.float32(image)
    dctM = dctn(image)
    dctRvec = dctM[0:5, 0:5].flatten()
    return dctRvec
